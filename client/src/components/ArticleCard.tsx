    ...
    return (
      <Link href={`/article/${article.slug}`}>
        <article className="group cursor-pointer">
          <div className="grid grid-cols-1 md:grid-cols-[1.15fr_0.95fr] gap-8 items-center">
            <div className="order-2 md:order-1">
              ...
              <p
                className="text-[#4A4A4A] dark:text-[#D0D0D0] text-sm leading-relaxed line-clamp-4"
                style={{ fontFamily: "'Noto Serif SC', serif" }}
              >
                {article.excerpt}
              </p>
            ...
            {article.coverImage ? (
              <div className="order-1 md:order-2 overflow-hidden rounded-2xl aspect-[4/3] md:aspect-[5/4] bg-muted ring-1 ring-foreground/5 shadow-[0_18px_50px_-28px_rgba(0,0,0,0.5)] transition-transform duration-500 group-hover:-translate-y-1 group-hover:shadow-[0_24px_60px_-24px_rgba(0,0,0,0.55)]">
                <img
                  src={article.coverImage}
                  alt={coverAlt}
                  className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.06]"
